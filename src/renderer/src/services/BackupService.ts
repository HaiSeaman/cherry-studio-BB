...
export async function backupToWebdav({
  ...
  try {
    const success = await window.api.backup.backupToWebdav({
      ...
      allowSelfSignedCertificate: webdavAllowSelfSignedCertificate
    })
    if (success) {
      store.dispatch(
        setWebDAVSyncState({
      ...
      showMessage && window.toast.success('备份成功')
      ...
      showMessage && window.toast.error('备份失败')
    }
  } catch (error: any) {
    // if auto backup process, throw error
    if (autoBackupProcess) {
