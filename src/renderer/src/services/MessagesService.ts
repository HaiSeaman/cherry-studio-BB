...
export function isGenerating() {
  return new Promise((resolve, reject) => {
    ...
    generating && window.toast.warning('请等待当前回复完成后操作')
...
export async function getMessageTitle(message: Message, length = 30): Promise<string> {
  ...
  if ((store.getState().settings as any).useTopicNamingForMessageTitle) {
    try {
      ...
      window.toast.loading({ title: '正在生成标题...', promise: titlePromise })
      ...
      if (title) {
        window.toast.success('标题生成成功')
        return title
      }
    } catch (e) {
      window.toast.error('标题生成失败，使用默认标题')
...
export function checkRateLimit(assistant: Assistant): boolean {
  ...
  if (timeDiff < rateLimitMs) {
    ...
    window.toast.warning(`发送过于频繁，请等待 ${waitTimeSeconds} 秒后再尝试`)
    return true
  }
