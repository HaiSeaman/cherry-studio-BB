...
export class DexieMessageDataSource implements MessageDataSource {
  ...
  async fetchMessages(topicId: string): Promise<{
    ...
    try {
      const topic = await db.transaction('rw', db.topics, async () => {
        // get-then-add inside a single transaction so two windows fetching the
        // same brand-new topic cannot race into a ConstraintError.
        const existing = await db.topics.get(topicId)
        if (!existing) {
          await db.topics.add({ id: topicId, messages: [] })
          return { id: topicId, messages: [] }
        }
        return existing
      })
  ...
  async clearMessages(topicId: string): Promise<void> {
    try {
      // Collect block ids / files AND clear the topic in ONE write transaction
      // so no message appended between the old read and write transactions can
      // be silently dropped (its blocks would also be left orphaned).
      let files: any[] = []
  ...
  async ensureTopic(topicId: string): Promise<void> {
    try {
      await db.transaction('rw', db.topics, async () => {
        // Atomic get-then-add: prevents ConstraintError when two windows
        // ensure the same new topic concurrently.
        const exists = await db.topics.get(topicId)
        if (!exists) {
          await db.topics.add({ id: topicId, messages: [] })
        }
      })
  ...
  async updateFileCount(fileId: string, delta: number, deleteIfZero: boolean = false): Promise<void> {
    let extToDelete: string | undefined
    try {
      await db.transaction('rw', db.files, async () => {
        ...
        if (newCount <= 0 && deleteIfZero) {
          // Delete the DB record here; physical file deletion happens after
          // the transaction (FileManager.deleteFile would open a nested
          // transaction and also re-delete the now-missing record).
          extToDelete = file.ext
      ...
      if (extToDelete !== undefined) {
        try {
          await window.api.file.delete(fileId + extToDelete)
        } catch (error) {
          logger.error('Failed to delete physical file after count reached zero:', error as Error)
        }
      }
    } catch (error) {
