...
describe('pdfCompatibilityPlugin', () => {
  ...
  it('should convert PDF for Gemini model accessed through non-Gemini provider (e.g., custom aggregator)', async () => {
    // A Gemini-named model through an OpenAI-compatible aggregator (type: 'openai') uses
    // OpenAI-compatible format which does NOT support native PDF file parts (would cause 400
    // "type has to be either 'image_url' or 'text'" errors). PDF must be converted to text.
