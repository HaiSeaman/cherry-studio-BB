...
describe('Auto-Extracted Type System', () => {
  describe('Runtime and Type Consistency', () => {
    it('运行时 IDs 应该自动提取到类型系统', () => {
      ...
      // 验证别名
      const expectedAliases: AppProviderId[] = ['aws-bedrock', 'hf', 'hugging-face', 'ai-gateway']
  ...
  describe('Type Inference - Auto-Extracted', () => {
    ...
    it('应该接受项目特定 provider IDs', () => {
      ...
      // 运行时验证
      expect(projectIds.length).toBe(7)
    })

    it('应该接受项目特定 provider 别名', () => {
      // 编译时类型检查 - 别名也自动提取
      const aliases: AppProviderId[] = ['aws-bedrock', 'hf', 'hugging-face', 'ai-gateway']
    ...
    it('AppProviderId 应该包含项目和核心的所有 IDs', () => {
      ...
      type Check1 = 'ollama' extends AppProviderId ? true : false

      // ✅ 核心 IDs 也应该在 AppProviderId 中（统一类型系统）
      type Check2 = 'openai' extends AppProviderId ? true : false
      type Check3 = 'anthropic' extends AppProviderId ? true : false
