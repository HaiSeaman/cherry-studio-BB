...
export default class AiProvider {
   ...
   * Gemini 图像模型（Google 官方接口）走官方 aspectRatio + imageSize(1K/2K/4K) 参数；
   * 其余模型（含 OpenAI 兼容接口的 gemini 模型）走通用像素 size 参数
   */
  private async modernGenerateImage(params: GenerateImageParams, providerConfig: ProviderConfig): Promise<string[]> {
    const { model, prompt, imageSize, batchSize, aspectRatio, personGeneration, signal } = params
    ...
    const isGemini = isGeminiImageModelId(model) && this.actualProvider.type === 'gemini'

    if (isGemini) {
      // Gemini 官方参数：imageConfig.aspectRatio + imageConfig.imageSize + personGeneration
      // （AI SDK gemini imageModel 对 providerOptions.google 直接透传，不校验 schema）
      const result = await executor.generateImage({
        model,
        prompt,
        n: 1, // Gemini 图像模型不支持单次多张
        providerOptions: {
          google: {
            imageConfig: {
   ...
   * Gemini 图像模型（Google 官方接口）走官方 imageConfig.aspectRatio（且不传像素 size）
   */
  private async modernEditImage(params: EditImageParams, providerConfig: ProviderConfig): Promise<string[]> {
    const { model, prompt, inputImages, mask, imageSize, aspectRatio, signal } = params
  ...
  /**
   * 转换图像生成结果格式
   * AI SDK v6 的 GeneratedFile 统一为 base64 格式（SDK 内部已转换 URL 返回）
   */
