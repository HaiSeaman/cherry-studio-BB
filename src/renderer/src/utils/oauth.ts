...
/** 打开服务商的充值页面（通用实现：跳转服务商官网） */
export const providerCharge = (providerId: string) => {
  const url = PROVIDER_URLS[providerId as keyof typeof PROVIDER_URLS]?.websites?.official
  if (url) {
    window.open(url, '_blank')
  }
}

/** 打开服务商的账单页面（通用实现：跳转服务商官网） */
export const providerBills = (providerId: string) => {
  const url = PROVIDER_URLS[providerId as keyof typeof PROVIDER_URLS]?.websites?.official
  if (url) {
    window.open(url, '_blank')
  }
...
export const oauthWithAihubmix = async (setKey) => {
  const authUrl = ` https://console.inferera.com/token?client_id=cherry_studio_oauth&lang=zh-CN&aff=SJyh`
