...
export const exportTopicAsMarkdown = async (
  ...
  if (getExportState()) {
    window.toast.warning('正在进行其他导出，请等待上一导出完成后重试')
    return
  }
  ...
  if (!markdownExportPath) {
    try {
      ...
      if (result) {
        window.toast.success('成功导出 Markdown 文件')
      }
    } catch (error: any) {
      window.toast.error('导出 Markdown 文件失败')
    ...
    try {
      ...
      window.toast.success('成功导出 Markdown 文件到预先设定的路径')
    } catch (error: any) {
      window.toast.error('导出 Markdown 文件到预先设定的路径失败')
      logger.error('Failed to export topic as markdown:', error)
    } finally {
      setExportingState(false)
    }
...
const executeNotionExport = async (title: string, allBlocks: any[]): Promise<boolean> => {
  ...
  if (!notionApiKey || !notionDatabaseID) {
    window.toast.error('未配置 Notion API Key 或 Notion Database ID')
    return false
  }

  if (allBlocks.length === 0) {
    window.toast.error('导出 Notion 错误，请检查连接状态并对照文档检查配置')
    return false
  }
  ...
  try {
    ...
    window.toast.loading({ title: '正在准备导出到 Notion...', promise: responsePromise })
    ...
    window.toast.loading({ title: '正在导出到 Notion ...', promise: exportPromise })

    window.toast.success('成功导出到 Notion')
    return true
  } catch (error: any) {
    // 清理可能存在的loading消息
...
export const exportMarkdownToYuque = async (title: string, content: string): Promise<any | null> => {
  ...
  if (!yuqueToken || !yuqueRepoId) {
    window.toast.error('未配置语雀 Token 或 知识库 URL')
    return
  }
  ...
  try {
    ...
    window.toast.success('成功导出到语雀')
    return data
  } catch (error: any) {
    logger.debug(error)
    window.toast.error('导出语雀错误，请检查连接状态并对照文档检查配置')
    return null
  } finally {
    setExportingState(false)
  }
...
export const exportMarkdownToObsidian = async (attributes: any): Promise<void> => {
  ...
  try {
    ...
    if (!obsidianVault) {
      window.toast.error('请先选择一个保管库')
      return
    }

    if (!attributes.title) {
      window.toast.error('标题不能为空')
      return
    }
    ...
    window.toast.success('导出到 Obsidian 成功')
  } catch (error) {
...
export const exportMarkdownToJoplin = async (
  ...
  if (!joplinUrl || !joplinToken) {
    window.toast.error('未配置 Joplin 授权令牌 或 URL')
    return
  }
  ...
  try {
    ...
    window.toast.success('成功导出到 Joplin')
    return data
  } catch (error: any) {
    logger.error('Failed to export to Joplin:', error)
    window.toast.error('导出 Joplin 失败，请保持 Joplin 已运行并检查连接状态或检查配置')
    return null
  } finally {
    setExportingState(false)
  }
...
export const exportMarkdownToSiyuan = async (title: string, content: string): Promise<void> => {
  ...
  if (!siyuanApiUrl || !siyuanToken || !siyuanBoxId) {
    window.toast.error('未配置思源笔记 API 地址或令牌')
    return
  }
  ...
  try {
    ...
    if (testData.code !== 0) {
      throw new Error(`${testData.msg || '未知错误'}`)
    }
    ...
    window.toast.success('导出到思源笔记成功')
  } catch (error) {
    logger.error('Failed to export to Siyuan:', error as Error)
    window.toast.error(
      '导出思源笔记失败，请检查连接状态并对照文档检查配置' + (error instanceof Error ? `: ${error.message}` : '')
    )
  } finally {
    setExportingState(false)
  }
