...
describe('WebviewSearch', () => {
  ...
  it('performs searches and navigates between results', async () => {
    ...
    const nextButton = screen.getByRole('button', { name: '下一个匹配' })
    await waitFor(() => {
    ...
    const previousButton = screen.getByRole('button', { name: '上一个匹配' })
  ...
  it('shows toast error when search fails', async () => {
    ...
    await waitFor(() => {
      expect(toastMock.error).toHaveBeenCalledWith('错误')
    })
  })
