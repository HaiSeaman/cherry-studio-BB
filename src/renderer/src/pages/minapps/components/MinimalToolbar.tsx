  ...
  return (
    <ToolbarContainer>
      <LeftSection>
        <ButtonGroup>
          <Tooltip title={'后退'} placement="bottom">
            <ToolbarButton onClick={handleGoBack} $disabled={!canGoBack} aria-label={'后退'} aria-disabled={!canGoBack}>
              <ArrowLeftOutlined />
            </ToolbarButton>
          </Tooltip>
          ...
          <Tooltip title={'刷新'} placement="bottom">
            <ToolbarButton onClick={onReload} aria-label={'刷新'}>
              <ReloadOutlined />
            </ToolbarButton>
          </Tooltip>
        </ButtonGroup>
      ...
      <RightSection>
        <ButtonGroup>
          {canOpenExternalLink && (
            <Tooltip title={'在浏览器中打开'} placement="bottom">
              <ToolbarButton onClick={handleOpenLink} aria-label={'在浏览器中打开'}>
                <ExportOutlined />
              </ToolbarButton>
            </Tooltip>
          )}
          ...
          {canPinned && (
            <Tooltip title={isPinned ? '从启动台移除' : '添加到启动台'} placement="bottom">
              <ToolbarButton
                onClick={handleTogglePin}
                $active={isPinned}
                aria-label={isPinned ? '从启动台移除' : '添加到启动台'}
          ...
          <Tooltip
            title={minappsOpenLinkExternal ? '当前：在浏览器中打开链接' : '当前：使用默认窗口打开链接'}
            placement="bottom">
            <ToolbarButton
              onClick={handleToggleOpenExternal}
              $active={minappsOpenLinkExternal}
              aria-label={minappsOpenLinkExternal ? '当前：在浏览器中打开链接' : '当前：使用默认窗口打开链接'}
          ...
          {isDev && (
            <Tooltip title={'开发者工具'} placement="bottom">
              <ToolbarButton onClick={onOpenDevTools} aria-label={'开发者工具'}>
                <CodeOutlined />
              </ToolbarButton>
            </Tooltip>
          )}

          <Tooltip title={'最小化小程序'} placement="bottom">
            <ToolbarButton onClick={handleMinimize} aria-label={'最小化小程序'}>
              <MinusOutlined />
            </ToolbarButton>
          </Tooltip>
        </ButtonGroup>
