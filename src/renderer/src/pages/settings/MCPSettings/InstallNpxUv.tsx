...
const InstallNpxUv: FC<Props> = ({ mini = false }) => {
  ...
  const installUV = async () => {
    try {
      ...
      window.toast.error(`${'安装依赖项失败'}: ${error.message}`)
      setIsInstallingUv(false)
    }
    clearTimeout(checkBinariesTimerRef.current)
          ...
          <VStack>
            <SettingRow style={{ width: '100%' }}>
              <SettingSubtitle style={{ margin: 0, fontWeight: 'normal' }}>
                {isUvInstalled ? 'UV Installed' : `UV ${'缺失，请安装它以继续'}`}
              </SettingSubtitle>
              {!isUvInstalled && (
                <Button
                  type="primary"
      ...
      <Alert
        ...
        description={
          <VStack>
            <SettingRow style={{ width: '100%' }}>
              <SettingSubtitle style={{ margin: 0, fontWeight: 'normal' }}>
                {isBunInstalled ? 'Bun Installed' : `Bun ${'缺失，请安装它以继续'}`}
              </SettingSubtitle>
              {!isBunInstalled && (
                <Button
                  type="primary"
