...
const McpProviderSettings: React.FC<Props> = ({ provider, existingServers }) => {
  ...
  const handleFetch = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        window.toast.success('服务器获取成功')
      } else {
        window.toast.error(result.message)
      }
    } catch (error: any) {
      logger.error('Failed to fetch MCP servers', error)
      window.toast.error(`${'同步 MCP 服务器出错'}: ${error.message}`)
    } finally {
      setIsFetching(false)
    }
  }, [existingServers, provider, token])
  ...
  return (
    <DetailContainer>
      ...
      <SettingSubtitle style={{ marginTop: 5 }}>{'API 密钥'}</SettingSubtitle>
      ...
      {sortedServers.length > 0 && (
        <>
          <Flex justify="space-between" align="center" style={{ marginTop: 20 }}>
            <SettingSubtitle style={{ margin: 0 }}>{'MCP 服务器'}</SettingSubtitle>
