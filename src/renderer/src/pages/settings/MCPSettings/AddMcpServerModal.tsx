...
const AddMcpServerModal: FC<AddMcpServerModalProps> = ({
  ...
  const handleOk = async () => {
    try {
      ...
      if (importMethod === 'dxt') {
        if (!dxtFile) {
          window.toast.error('请选择一个 DXT 文件')
          setLoading(false)
          return
        }
        ...
        try {
          ...
          if (!result.success) {
            window.toast.error(result.error || '处理 DXT 文件失败')
            setLoading(false)
            return
          }
          ...
          if (existingServers && existingServers.some((server) => server.name === manifest.name)) {
            window.toast.error(`服务器已存在：${manifest.name}`)
            setLoading(false)
            return
          }
          ...
          window.toast.error('处理 DXT 文件失败')
          setLoading(false)
          return
        }
      } else {
        ...
        if (existingServers && existingServers.some((server) => server.name === serverToAdd.name)) {
          form.setFields([
            {
              name: 'serverConfig',
              errors: [`服务器已存在：${serverToAdd.name}`]
            }
          ])
          setLoading(false)
          return
        ...
        window.api.mcp
          ...
          .catch((connError: any) => {
            logger.error(`Connectivity check failed for ${newServer.name}:`, connError)
            window.toast.error(newServer.name + '连接失败')
          })
      }
    } finally {
      setLoading(false)
  ...
  return (
    <Modal
      title={importMethod === 'dxt' ? '导入 DXT 包' : '从 JSON 导入'}
      open={visible}
      onOk={handleOk}
      onCancel={() => {
        form.resetFields()
      ...
      <Form form={form} layout="vertical" name="add_mcp_server_form">
        {importMethod === 'json' ? (
          <Form.Item
            name="serverConfig"
            label={'请从 MCP Servers 的介绍页面复制配置 JSON（优先使用\n NPX 或 UVX 配置），并粘贴到输入框中'}
            rules={[{ required: true, message: '粘贴 MCP 服务器 JSON 配置' }]}>
            <CodeEditor
          ...
          <Form.Item label={'DXT 包文件'} help={'选择包含 MCP 服务器的 .dxt 文件'}>
            <Upload
              accept=".dxt"
              maxCount={1}
              beforeUpload={(file) => {
              ...
              <Button icon={<UploadOutlined />}>{'选择 DXT 文件'}</Button>
            </Upload>
          </Form.Item>
        )}
      </Form>
