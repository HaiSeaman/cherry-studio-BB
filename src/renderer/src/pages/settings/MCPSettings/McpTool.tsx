...
const MCPToolsSection = ({ tools, server, onToggleTool, onToggleAutoApprove }: MCPToolsSectionProps) => {
      ...
      <Flex vertical gap={4}>
        ...
        {prop.enum && (
          <div style={{ marginTop: 4 }}>
            <Typography.Text type="secondary">{'允许的值'}</Typography.Text>
  ...
  const columns: ColumnsType<MCPTool> = [
    {
      title: <Typography.Text strong>{'可用工具'}</Typography.Text>,
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      title: (
        <Flex align="center" justify="center" gap={4}>
          ...
          <Typography.Text strong>{'启用工具'}</Typography.Text>
        </Flex>
      ),
      key: 'enable',
    ...
    },
    {
      title: (
        <Flex align="center" justify="center" gap={4}>
          ...
          <Typography.Text strong>{'自动批准'}</Typography.Text>
        </Flex>
      ),
      key: 'autoApprove',
  ...
  return tools.length > 0 ? (
    ...
    <Empty description={'无可用工具'} image={Empty.PRESENTED_IMAGE_SIMPLE} />
  )
}
