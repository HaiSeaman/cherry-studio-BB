...
const SelectionActionSearchModal: FC<SelectionActionSearchModalProps> = ({
  ...
  return (
    <Modal title={'设置搜索引擎'} open={isModalOpen} onOk={handleOk} onCancel={handleCancel} destroyOnHidden centered>
      <Form
        form={form}
        layout="vertical"
        initialValues={{
            ...
            getFieldValue('engine') === 'custom' ? (
              <>
                <Form.Item
                  ...
                  label={'自定义名称'}
                  rules={[
                    { required: true, message: '请输入搜索引擎名称' },
                    { max: 16, message: '名称不能超过 16 个字符' }
                  ]}>
                ...
                <Form.Item
                  name="customUrl"
                  label={'自定义搜索 URL'}
                  tooltip={'用 {{queryString}} 代表搜索词'}
                  rules={[
                    { required: true, message: '请输入搜索 URL' },
                    {
                      pattern: /^https?:\/\/.+$/,
                      message: '请输入以 http:// 或 https:// 开头的有效 URL'
                    },
                    {
                      validator: (_, value) => {
                        if (value && !value.includes('{{queryString}}')) {
                          return Promise.reject('URL 必须包含 {{queryString}} 占位符')
                        }
