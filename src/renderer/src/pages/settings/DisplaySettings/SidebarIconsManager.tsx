...
import { Image as ImageIcon, LayoutGrid, MessageSquareQuote, Music, StickyNote } from 'lucide-react'
...
const SidebarIconsManager: FC<SidebarIconsManagerProps> = ({
  ...
  const onDragEnd = useCallback(
    ...
    [visibleIcons, disabledIcons, dispatch, setVisibleIcons, setDisabledIcons]
  )
  ...
  const onMoveIcon = useCallback(
    ...
    [visibleIcons, disabledIcons, setVisibleIcons, setDisabledIcons, dispatch]
  )
            ...
            {(provided: DroppableProvided) => (
              <IconList ref={provided.innerRef} {...provided.droppableProps}>
                {disabledIcons.length === 0 ? (
                  <EmptyPlaceholder>{'把要隐藏的功能从左侧拖拽到这里'}</EmptyPlaceholder>
                ) : (
