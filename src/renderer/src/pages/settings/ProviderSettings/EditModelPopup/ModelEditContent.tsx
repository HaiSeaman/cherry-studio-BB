...
const ModelEditContent: FC<ModelEditContentProps & ModalProps> = ({ provider, model, onUpdateModel, ...props }) => {
      ...
      <>
        <TypeTitle>
          <Flex align="center" gap={4} style={{ height: 24 }}>
            {'模型类型'}
            <WarnTooltip title={'请慎重更改模型类型，选择错误的类型会导致模型无法正常使用！'} />
          </Flex>
  ...
  return (
    <Modal title={'编辑模型'} footer={null} transitionName="animation-move-down" centered {...props}>
      <Form
        form={form}
        ...
        {showMoreSettings && (
          <div style={{ marginBottom: 8 }}>
            ...
            <Form.Item name="currencySymbol" label={'币种'} style={{ marginBottom: 10 }}>
              <Select
                style={{ width: '100px' }}
            ...
            <Form.Item label={'输入价格'} style={{ marginBottom: 10 }} name="input_per_million_tokens">
              <InputNumber
                placeholder="0.00"
                ...
                precision={2}
                style={{ width: '240px' }}
                addonAfter={`${currencySymbol} / ${'百万 Token'}`}
                onChange={() => {
                  // 自动保存
                  autoSave()
                }}
            ...
            <Form.Item label={'输出价格'} style={{ marginBottom: 10 }} name="output_per_million_tokens">
              <InputNumber
                placeholder="0.00"
