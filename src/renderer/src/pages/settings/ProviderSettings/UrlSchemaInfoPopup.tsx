...
const PopupContainer = ({ id, apiKey: newApiKey, baseUrl, type, name, resolve }: Props) => {
  ...
  const okText = keyAlreadyExists ? '确认' : '添加'
    ...
    <Modal
      ...
      <Container>
        <Descriptions column={1} size="small" bordered>
          <Descriptions.Item label={'服务商名称'}>{displayName}</Descriptions.Item>
          <Descriptions.Item label={'服务商 ID'}>{baseProvider.id}</Descriptions.Item>
          {baseUrl && <Descriptions.Item label={'基础 URL'}>{baseUrl}</Descriptions.Item>}
