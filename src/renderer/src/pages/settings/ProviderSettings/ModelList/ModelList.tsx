...
const ModelList: React.FC<ModelListProps> = ({ providerId }) => {
  ...
  const onAddModel = useCallback(() => {
    if (isNewApiProvider(provider)) {
      void NewApiAddModelPopup.show({ title: '添加模型', provider })
    } else {
      void AddModelPopup.show({ title: '添加模型', provider })
    }
  }, [provider])
  ...
  const actionButtons = (
    <Space.Compact>
      ...
      <Tooltip title={'添加'} mouseLeaveDelay={0}>
      ...
      <SettingSubtitle style={{ marginBottom: 12 }}>
        <HStack alignItems="center" justifyContent="space-between" style={{ width: '100%' }}>
          <HStack alignItems="center" gap={8}>
            <SettingSubtitle style={{ marginTop: 0 }}>{'模型'}</SettingSubtitle>
            <CustomTag color="#8c8c8c" size={10}>
              {modelCount}
            </CustomTag>
            {!hasNoModels && (
              <>
                <Tooltip title={'健康检测'} mouseLeaveDelay={0}>
                  <Button
                    type="text"
                ...
                <CollapsibleSearchBar onSearch={setSearchText} placeholder={'搜索模型...'} tooltip={'搜索模型'} />
              </>
            )}
          </HStack>
          {actionButtons}
      ...
      <Flex justify="space-between" align="center">
        {docsWebsite || modelsWebsite ? (
          <SettingHelpTextRow>
            ...
            {docsWebsite && modelsWebsite && <SettingHelpText>{'和'}</SettingHelpText>}
            {modelsWebsite && (
