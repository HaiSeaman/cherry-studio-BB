...
const PopupContainer: React.FC<Props> = ({ providerId, resolve }) => {
    ...
    (model: Model) => {
      if (!isEmpty(model.name)) {
        ...
        if (isNewApiProvider(provider) && !hasSupportedEndpointTypes) {
          void NewApiAddModelPopup.show({ title: '添加模型', provider, model })
          return
        }
  ...
  const renderTopTools = useCallback(() => {
    ...
    return (
      <HStack gap={8}>
        <Tooltip title={isAllFilteredInProvider ? '移除列表中的模型' : '添加列表中的模型'} mouseLeaveDelay={0}>
          <Button
            type="default"
        ...
        <Tooltip title={'重新获取模型列表'} mouseLeaveDelay={0}>
          <Button
            type="default"
  ...
  }, [list, loadingModels, provider, onRemoveAll, onAddAll, loadModels])
    ...
    <Modal
      ...
      <SearchContainer>
        ...
        <Tabs
          ...
          items={[
            { label: '全部', key: 'all' },
            { label: '推理', key: 'reasoning' },
            { label: '视觉', key: 'vision' },
            { label: '联网', key: 'websearch' },
            { label: '免费', key: 'free' },
            { label: '嵌入', key: 'embedding' },
            { label: '重排', key: 'rerank' },
            { label: '工具', key: 'function_calling' }
          ]}
          onChange={(key) => {
