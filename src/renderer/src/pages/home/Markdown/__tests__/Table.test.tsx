...
| Cell 1   | Cell 2   |`
  ...
  const getCopyButton = () => screen.getByRole('button', { name: /复制/i })
  const getExcelButton = () => screen.getByRole('button', { name: /导出为 Excel/i })
  ...
  describe('rendering', () => {
    ...
    it('should render copy button with correct tooltip', () => {
      ...
      expect(tooltips[0]).toHaveAttribute('title', '复制')
    })
    ...
    it('should render excel export button with correct tooltip', () => {
      ...
      const tooltips = screen.getAllByTestId('tooltip')
      expect(tooltips[1]).toHaveAttribute('title', '导出为 Excel')
...
| Cell 1   | Cell 2   |`)
  ...
  describe('copy functionality', () => {
    ...
    it('should show error toast when extractTableMarkdown returns empty string', async () => {
      ...
      await waitFor(() => {
        expect(mocks.windowToast.error).toHaveBeenCalledWith('无法获取有效的表格数据')
        expect(getCopyIcon()).toBeInTheDocument()
        expect(queryCheckIcon()).not.toBeInTheDocument()
      })
    })
  ...
  describe('excel export functionality', () => {
    ...
    it('should show success toast after successful export', async () => {
      ...
      await waitFor(() => {
        expect(mocks.windowToast.success).toHaveBeenCalledWith('Excel 导出成功')
      })
    })
    ...
    it('should show error toast and log error on export failure', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mocks.windowToast.error).toHaveBeenCalledWith('导出 Excel 失败')
      })
    })
