...
const Table: React.FC<Props> = ({ children, node, blockId }) => {
  ...
  const handleCopyTable = useCallback(async () => {
    ...
    if (!tableMarkdown) {
      window.toast?.error('无法获取有效的表格数据')
      return
    }
  ...
  }, [blockId, node?.position, setCopied])
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      if (result) {
        window.toast?.success('Excel 导出成功')
      }
    } catch (error) {
      logger.error('Failed to export table to Excel', { error })
      window.toast?.error('导出 Excel 失败')
    }
  }, [blockId, node?.position])
    ...
    <TableWrapper className="table-wrapper">
      ...
      <ToolbarWrapper className="table-toolbar">
        <Tooltip title={'复制'} mouseEnterDelay={0.8}>
          <ToolButton role="button" aria-label={'复制'} onClick={handleCopyTable}>
            {copied ? <Check size={14} color="var(--color-primary)" /> : <CopyIcon size={14} />}
          </ToolButton>
        </Tooltip>
        <Tooltip title={'导出为 Excel'} mouseEnterDelay={0.8}>
          <ToolButton role="button" aria-label={'导出为 Excel'} onClick={handleExportExcel}>
