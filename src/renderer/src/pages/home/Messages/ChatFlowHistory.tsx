    ...
    <Tooltip
      title={
        <TooltipContent>
          ...
          <TooltipFooter>{'点击跳转到对应消息'}</TooltipFooter>
        </TooltipContent>
      }
      placement="top"
...
const ChatFlowHistory: FC<ChatFlowHistoryProps> = ({ conversationId }) => {
  ...
  const buildConversationFlowData = useCallback(() => {
    ...
    userMessages.forEach((message, index) => {
      ...
      // 获取用户名
      const userNameValue = userName || '用户'
      ...
      relatedAssistantMsgs.forEach((aMsg, aIndex) => {
        ...
        // 获取模型名称
        const modelName = (aMsgAny.model && aMsgAny.model.name) || '助手'
  ...
  }, [topicId, messages, userMessages, assistantMessages])
