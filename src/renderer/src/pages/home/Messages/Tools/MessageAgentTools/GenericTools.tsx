...
export function ToolStatusIndicator({ status, hasError = false }: { status: ToolStatus; hasError?: boolean }) {
  const getStatusInfo = (): { label: string; icon: ReactNode; color: StatusColor } | null => {
    switch (status) {
      case 'streaming':
        return { label: '流式传输中', icon: <LoadingIcon />, color: 'primary' }
      case 'waiting':
        return { label: '等待中', icon: <LoadingIcon />, color: 'warning' }
      case 'pending':
      case 'invoking':
        return { label: '调用中', icon: <LoadingIcon />, color: 'primary' }
      case 'cancelled':
        return {
          label: '已取消',
...
export function TruncatedIndicator({ originalLength }: { originalLength: number }) {
  ...
  return (
    <div className="mt-2 flex items-center gap-1 text-muted-foreground text-xs">
      <Ellipsis size={14} />
      <span className="rounded bg-muted px-1.5 py-0.5 font-mono">{`输出已截断（原始大小：${sizeStr}）`}</span>
    </div>
  )
}
