...
const MessageMcpTool: FC<Props> = ({ block }) => {
  ...
  const copyContent = (content: string, toolId: string) => {
    ...
    window.toast.success({ title: '已复制', key: 'copy-message' })
  ...
  const handleAbortTool = async () => {
    if (toolResponse?.id) {
      try {
        ...
        if (success) {
          window.toast.success('工具调用已中断')
        } else {
          message.error({ content: '工具调用中断失败', key: 'abort-tool' })
        }
      } catch (error) {
          ...
          <TitleContent>
            ...
            <ToolName align="center" gap={4}>
              ...
              {isToolAutoApproved(tool) && (
                <Tooltip title={'此工具已启用自动批准'} mouseLeaveDelay={0}>
        ...
        <ToolContainer>
          <ToolContentWrapper className={isPending || approval.isWaiting ? 'pending' : status}>
            ...
            {(isPending || approval.isWaiting || approval.isExecuting) && (
              <ActionsBar>
                <ActionLabel>{approval.isWaiting ? '是否运行该MCP工具？' : '调用中'}</ActionLabel>
