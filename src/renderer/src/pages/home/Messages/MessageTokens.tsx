...
const MessageTokens: React.FC<MessageTokensProps> = ({ message }) => {
  ...
  const getPriceString = () => {
    ...
    return `| ${'花费'}: ${currencySymbol}${price.toFixed(6)}`
  }
  ...
  if (message.role === 'assistant') {
    ...
    if (message?.metrics?.completion_tokens && message?.metrics?.time_completion_millsec) {
      hasMetrics = true
      metrixs = `首字时延 ${message?.metrics?.time_first_token_millsec} ms | 每秒 ${(
        message?.metrics?.completion_tokens / (message?.metrics?.time_completion_millsec / 1000)
      ).toFixed(0)} tokens`
    }
