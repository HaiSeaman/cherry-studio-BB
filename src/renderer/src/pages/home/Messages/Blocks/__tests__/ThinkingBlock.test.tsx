...
describe('ThinkingBlock', () => {
  beforeEach(async () => {
    ...
    mockUseTranslation.mockReturnValue({
      t: (key: string, params?: any) => {
        if (key === '思考中（用时 {{seconds}} 秒）' && params?.seconds) {
          return `Thinking... ${params.seconds}s`
        }
        if (key === '已深度思考（用时 {{seconds}} 秒）' && params?.seconds) {
          return `Thought for ${params.seconds}s`
        }
        if (key === '已复制') return 'Copied!'
        if (key === '复制失败') return 'Copy failed'
        if (key === '复制') return 'Copy'
        return key
      }
    })
  })
  ...
  const getCopyButton = () => screen.queryByRole('button', { name: /复制/i })
  ...
  describe('thinking time display', () => {
    it('should display appropriate time messages based on status', () => {
      ...
      expect(timeText).toHaveTextContent('用时 3.5 秒')
      expect(timeText).toHaveTextContent('已深度思考')
      unmount()
      ...
      expect(activeTimeText).toHaveTextContent('思考中')
    })

    it('should handle extreme thinking times correctly', () => {
      const testCases = [
        { thinking_millsec: 0, expectedTime: '用时 0.1 秒' }, // New logic: values < 1000ms display as 0.1s
        { thinking_millsec: 86400000, expectedTime: '用时 86400.0 秒' }, // 1 day
        { thinking_millsec: 259200000, expectedTime: '用时 259200.0 秒' } // 3 days
      ]
    ...
    it('should clamp invalid thinking times to a safe default', () => {
      ...
      testCases.forEach((thinking_millsec) => {
        ...
        const { unmount } = renderThinkingBlock(block)
        expect(getThinkingTimeText()).toHaveTextContent('用时 0.1 秒')
        unmount()
      })
    })
  })
