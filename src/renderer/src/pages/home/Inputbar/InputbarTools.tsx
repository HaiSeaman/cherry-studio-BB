...
const InputbarTools = ({ scope, assistant, model, session }: InputbarToolsNewProps) => {
  ...
  const buildRenderContext = useCallback(
    ...
    [assistant, model, quickPanelContext, scope, session, toolsContext, getQuickPanelApiForTool]
  )
  ...
  const getMenuItems = useMemo(() => {
    ...
    if (targetTool) {
      ...
      baseItems.push({
        label: `${targetTool.visible ? '加入折叠' : '移出折叠'} "${targetTool.label}"`,
    ...
    return baseItems
  }, [hiddenTools, targetTool, toggleToolVisibility, visibleTools])
