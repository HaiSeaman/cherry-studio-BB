...
const AssistantsTab: FC<AssistantsTabProps> = (props) => {
  const { activeAssistant, setActiveAssistant, onCreateAssistant } = props
  ...
  const groupedAssistantItems = useMemo(() => {
    ...
    assistants.forEach((assistant) => {
      const tags = assistant.tags?.length ? assistant.tags : ['未分组']
      tags.forEach((tag) => {
        if (!groups.has(tag)) {
          groups.set(tag, [])
        }
  ...
  const handleAssistantGroupReorder = useCallback(
    (tag: string, newGroupList: Assistant[]) => {
      ...
      const updatedAssistants = assistants.map((a) => {
        const tags = a.tags?.length ? a.tags : ['未分组']
  ...
  const onDeleteAssistant = useCallback(
    (assistant: Assistant) => {
      ...
      if (remaining.length === 0) {
        window.toast.error('不允许删除最后一个助手')
        return
      }
    ...
    [assistants, activeAssistant?.id, removeAssistant, setActiveAssistant]
  )
