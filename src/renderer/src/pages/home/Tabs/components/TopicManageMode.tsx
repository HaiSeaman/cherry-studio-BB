...
export const TopicManagePanel: React.FC<TopicManagePanelProps> = ({
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    if (remainingTopics.length === 0) {
      window.toast.error('至少需要保留一个话题')
      return
    }

    const confirmed = await window.modal.confirm({
      title: '删除话题',
      content: `确定要删除选中的 ${selectedIds.size} 个话题吗？此操作不可撤销。`,
      centered: true,
    ...
    if (successfulIds.size === idsArray.length) {
      window.toast.success(`已删除 ${successfulIds.size} 个话题`)
    } else if (successfulIds.size > 0) {
      window.toast.warning(`成功删除 ${successfulIds.size} 个，失败 ${idsArray.length - successfulIds.size} 个`)
    } else {
