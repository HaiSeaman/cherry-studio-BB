...
const migrateConfig = {
  ...
  '100': (state: RootState) => {
    try {
      ...
      state.assistants.assistants.forEach((assistant) => {
        // @ts-ignore legacy migration: knowledgeRecognition field removed
  ...
  '102': (state: RootState) => {
    try {
      state.settings.openAI = {
        // @ts-ignore it's a removed type. migrated on 177
        summaryText: 'off',
        serviceTier: 'auto',
        verbosity: 'medium'
      }
  ...
  '203': (state: RootState) => {
    try {
      // Legacy migration slot kept for version continuity.
      // Previous agents icon/tab injection was removed together with the agents feature.
  ...
  '205': (state: RootState) => {
    try {
      ...
      // Add anthropicApiHost to lmstudio and ollama providers for Claude API compatibility
      state.llm.providers.forEach((provider) => {
  ...
  '209': (state: RootState) => {
    try {
      // migrate to remove removed features (openclaw, code_tools) from sidebar
      if (state.settings && state.settings.sidebarIcons) {
        const removedIcons = ['openclaw', 'code_tools'] as any[]
        state.settings.sidebarIcons.visible = state.settings.sidebarIcons.visible.filter(
  ...
  '210': (state: RootState) => {
    try {
      // migrate to remove files page from sidebar
      if (state.settings && state.settings.sidebarIcons) {
        const removedIcons = ['files'] as any[]
        state.settings.sidebarIcons.visible = state.settings.sidebarIcons.visible.filter(
          (icon) => !removedIcons.includes(icon)
        )
        state.settings.sidebarIcons.disabled = state.settings.sidebarIcons.disabled.filter(
  ...
  '211': (state: RootState) => {
    try {
      // migrate to remove the agents feature from the sidebar
      if (state.settings && state.settings.sidebarIcons) {
        const removedIcons = ['agents'] as any[]
        state.settings.sidebarIcons.visible = state.settings.sidebarIcons.visible.filter(
          (icon) => !removedIcons.includes(icon)
        )
        state.settings.sidebarIcons.disabled = state.settings.sidebarIcons.disabled.filter(
  ...
  '212': (state: RootState) => {
    try {
      // Remove the removed built-in Python MCP server (@cherry/python) from persisted config.
      // The in-memory factory no longer supports it; leftover entries would fail activation.
      if (state.mcp && Array.isArray(state.mcp.servers)) {
        const before = state.mcp.servers.length
        state.mcp.servers = state.mcp.servers.filter((server) => server.name !== '@cherry/python')
        if (state.mcp.servers.length !== before) {
          logger.info(`migrate 212 success: removed ${before - state.mcp.servers.length} python server(s)`)
        }
      }
  ...
  '213': (state: RootState) => {
    try {
      // 启用图片生成（paint）侧边栏入口：为已有持久化 sidebarIcons 的用户追加 paint 图标
      if (state.settings && state.settings.sidebarIcons) {
        if (!state.settings.sidebarIcons.visible.includes('paint' as any)) {
          state.settings.sidebarIcons.visible = [...state.settings.sidebarIcons.visible, 'paint' as any]
        }
      }
  ...
  '215': (state: RootState) => {
    try {
      // 启用闹钟便签 Tab 侧边栏入口：为已有持久化 sidebarIcons 的用户追加 notes 图标
      if (state.settings && state.settings.sidebarIcons) {
