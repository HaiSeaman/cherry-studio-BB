...
const WindowControls: React.FC = () => {
  ...
  return (
    <WindowControlsContainer>
      <Tooltip title={'最小化'} placement="bottom" mouseEnterDelay={DEFAULT_DELAY}>
        <ControlButton onClick={handleMinimize} aria-label={'最小化'}>
          <Minus size={14} />
        </ControlButton>
      </Tooltip>
      <Tooltip title={isMaximized ? '还原' : '最大化'} placement="bottom" mouseEnterDelay={DEFAULT_DELAY}>
        <ControlButton onClick={handleMaximize} aria-label={isMaximized ? '还原' : '最大化'}>
          {isMaximized ? <WindowRestoreIcon size={14} /> : <Square size={14} />}
        </ControlButton>
      </Tooltip>
      <Tooltip title={'关闭'} placement="bottom" mouseEnterDelay={DEFAULT_DELAY}>
        <ControlButton $isClose onClick={handleClose} aria-label={'关闭'}>
          <X size={17} />
        </ControlButton>
      </Tooltip>
    </WindowControlsContainer>
