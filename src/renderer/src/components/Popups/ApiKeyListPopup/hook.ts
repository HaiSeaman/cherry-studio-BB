...
import type { Model, Provider, WebSearchProvider } from '@renderer/types'
...
export function useApiKeys({ provider, updateProvider }: UseApiKeysProps) {
  ...
  // 验证 API key 格式
  const validateApiKey = useCallback((key: string, existingKeys: string[] = []): ApiKeyValidity => {
    ...
    if (existingKeys.includes(trimmedKey)) {
      return { isValid: false, error: 'API 密钥已存在' }
    }
  ...
  const checkKeyConnectivity = useCallback(
    async (index: number): Promise<void> => {
      ...
      const model = isLlmProvider(provider) ? await getModelForCheck(provider) : undefined
      if (model === null) return

      await runConnectivityCheck(index, model)
    },
    [provider, keys, connectivityStates, runConnectivityCheck]
  )
  ...
  }, [provider, keys, runConnectivityCheck])
...
// 获取模型用于检查
async function getModelForCheck(provider: Provider): Promise<Model | null> {
