...
const GoogleLoginTip = ({
  ...
  // 使用直接的消息文本
  const message =
    '提示：如遇到Google登录提示"不受信任的浏览器"，请先在小程序列表中的Google小程序中完成账号登录，再在其它小程序使用Google登录'
  ...
  const Title = ({ appInfo, url }: { appInfo: AppInfo | null; url: string | null }) => {
    ...
    const handleCopyUrl = (event: any, url: string) => {
      ...
      navigator.clipboard
        .writeText(url)
        .then(() => {
          window.toast.success('URL ' + '复制成功')
        })
        .catch(() => {
          window.toast.error('URL ' + '复制失败')
        })
    }
    ...
    return (
      <TitleContainer style={{ backgroundColor: backgroundColor }}>
        ...
        {appInfo.canOpenExternalLink && (
          <Tooltip title={'在浏览器中打开'} mouseEnterDelay={0.8} placement="bottom">
        ...
        <ButtonsGroup
          ...
          <Tooltip title={'前进'} mouseEnterDelay={0.8} placement="bottom">
            <TitleButton onClick={() => handleGoForward(appInfo.id)}>
              <ArrowRightOutlined />
            </TitleButton>
          </Tooltip>
          <Tooltip title={'刷新'} mouseEnterDelay={0.8} placement="bottom">
          ...
          {isDev && (
            <Tooltip title={'开发者工具'} mouseEnterDelay={0.8} placement="bottom">
          ...
          {canMinimize && (
            <Tooltip title={'最小化小程序'} mouseEnterDelay={0.8} placement="bottom">
              <TitleButton onClick={() => handlePopupMinimize()}>
                <MinusOutlined />
              </TitleButton>
            </Tooltip>
          )}
          <Tooltip title={'关闭小程序'} mouseEnterDelay={0.8} placement="bottom">
