    ...
    <PreviewSection>
      {html.trim() ? (
        <PreviewFrame
          ...
          srcDoc={html}
          title={previewTitle}
          // NOTE: allow-same-origin is intentionally omitted. Combined with
          // allow-scripts it would let the untrusted artifact access the
          // parent document (and window.api) via the shared origin.
    ...
    <ModalHeader onDoubleClick={() => setIsFullscreen(!isFullscreen)} className={clsx({ drag: isFullscreen })}>
      ...
      <HeaderRight onDoubleClick={(e) => e.stopPropagation()}>
        <Dropdown
          ...
          <Tooltip title={'捕获页面'} mouseLeaveDelay={0}>
