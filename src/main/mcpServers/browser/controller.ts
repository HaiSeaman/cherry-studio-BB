...
export class CdpBrowserController {
  ...
  public async open(url: string, timeout = 10000, privateMode = false, newTab = false, showWindow = false) {
    // Security: only http(s) URLs may be opened. file://, data:, javascript:
    // etc. would let a prompt-injected tool call read local files or run
    // script with access to the page origin.
    let parsedUrl: URL
    try {
    ...
    if (parsedUrl.protocol !== 'http:' && parsedUrl.protocol !== 'https:') {
      throw new Error(`Unsupported URL protocol: ${parsedUrl.protocol}. Only http/https are allowed.`)
    }
