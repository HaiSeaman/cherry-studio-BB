...
export function resolveToolId(mapping: ToolNameMapping, nameOrId: string): string | undefined {
  ...
  if (isNamespacedToolId(nameOrId)) {
    // Only accept namespaced ids that exist in the mapping. Returning
    // attacker-supplied ids verbatim would bypass disabledTools filters and
    // reach tools that were never exposed to the model.
    return mapping.toJs.has(nameOrId) ? nameOrId : undefined
  }
