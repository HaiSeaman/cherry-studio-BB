...
describe('toolname', () => {
  ...
  describe('resolveToolId', () => {
    ...
    it('returns known namespaced ids as-is', () => {
      expect(resolveToolId(mapping, 'github__search_repos')).toBe('github__search_repos')
    })

    it('rejects unknown namespaced ids (security: no bypass of tool allowlist)', () => {
      // A namespaced id that is not in the mapping must be rejected instead of
      // passed through verbatim (previously this bypassed disabledTools filters).
      expect(resolveToolId(mapping, 'unknown__tool')).toBeUndefined()
    })
