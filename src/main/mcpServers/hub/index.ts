...
export class HubServer {
  ...
  private async handleExec(input: ExecInput) {
    ...
    // The exec tool executes arbitrary JavaScript in a worker thread. It is a
    // powerful primitive and must be explicitly enabled by the user; without
    // this, a prompt-injected tool call would reach code execution.
    if (!configManager.get(ConfigKeys.HubExecEnabled, false)) {
      logger.warn('Blocked hub exec tool call: exec is disabled (enable it in config hubExecEnabled)')
      return {
        content: [
          {
            type: 'text',
            text: JSON.stringify(
              {
                ...
                isError: true,
                error:
                  'The exec tool is disabled. Enable it explicitly in the app configuration (hubExecEnabled) before using it.'
              },
              null,
              2
            )
