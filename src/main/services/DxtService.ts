...
import { getMcpDir, getTempDir, validateZipEntries } from '@main/utils/file'
...
/** Maximum size of an uploaded DXT archive (prevents zip-bomb disk exhaustion). */
const MAX_DXT_FILE_SIZE = 500 * 1024 * 1024 // 500 MB
...
class DxtService {
  ...
  public async uploadDxt(_: Electron.IpcMainInvokeEvent, filePath: string): Promise<DxtUploadResult> {
    ...
    try {
      ...
      // Reject oversized archives before touching them
      const fileSize = fs.statSync(filePath).size
      if (fileSize > MAX_DXT_FILE_SIZE) {
        throw new Error('DXT file exceeds the maximum allowed size (500 MB)')
      }
      ...
      try {
        // Reject path traversal entries (zip-slip) before writing anything
