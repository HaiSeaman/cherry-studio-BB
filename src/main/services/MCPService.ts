...
import { getMCPServersFromRedux } from './mcp/utils'
...
class McpService {
  ...
  async initClient(server: MCPServer): Promise<Client> {
    ...
    const initPromise = (async () => {
      try {
        ...
        const client = new Client({ name: 'Cherry-Studio-BB', version: app.getVersion() }, { capabilities: {} })
        ...
        > => {
          ...
          if (isBuiltinMCPServer(server) && server.name !== BuiltinMCPServerNames.mcpAutoInstall) {
            ...
            // Defense-in-depth: only bare command names (npx, python, ...) or
            // absolute paths may be spawned. Relative paths (./evil, ../evil,
            // subdir/evil) would execute from the current working directory.
            const rawCommand = String(server.command)
            const isWindowsAbs = /^[a-zA-Z]:[\\/]/.test(rawCommand)
            const isPosixAbs = rawCommand.startsWith('/')
            const isBareName = !rawCommand.includes('/') && !rawCommand.includes('\\')
            if (!isWindowsAbs && !isPosixAbs && !isBareName) {
              throw new Error(`Unsafe MCP server command: ${rawCommand}`)
            }
  ...
  public async callTool(
    ...
    return await callToolFunc({ server, name, args })
  }
