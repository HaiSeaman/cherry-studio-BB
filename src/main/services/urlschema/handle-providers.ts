...
export async function handleProvidersProtocolUrl(url: URL) {
  switch (url.pathname) {
    case '/api-keys': {
      ...
      // Validate the decoded payload before handing it to the renderer so a
      // malicious site cannot inject an arbitrary baseUrl (credential
      // phishing / traffic hijacking via provider import).
      try {
        ...
        try {
          ...
          logger.error('handleProvidersProtocolUrl rejected: invalid baseUrl')
          return
        }
        if (parsedBaseUrl.protocol !== 'http:' && parsedBaseUrl.protocol !== 'https:') {
          logger.error('handleProvidersProtocolUrl rejected: baseUrl must be http(s)')
          return
        }
      } catch (error) {
        logger.error('handleProvidersProtocolUrl rejected: invalid payload', error as Error)
        return
      }
