...
/**
 * Safety cap for the total uncompressed size of a user-supplied archive.
 * Far beyond any legitimate backup size, but still bounds zip-bomb work.
 */
export const MAX_ZIP_EXTRACT_BYTES = 50 * 1024 * 1024 * 1024 // 50 GB

/**
 * Validate zip entry names & total uncompressed size BEFORE extraction.
 *
 * `node-stream-zip`'s extract() does not sanitize entry names (zip-slip):
 * an entry named `../../evil` resolves outside the target directory via
 * path.join. Callers must run this check before extract() and reject
 * archives whose entries escape the extraction root or exceed the size cap.
 */
export async function validateZipEntries(
  zip: StreamZip.StreamZipAsync,
  maxTotalSize: number = MAX_ZIP_EXTRACT_BYTES
): Promise<void> {
  const entries = await zip.entries()
  let totalSize = 0
  for (const name of Object.keys(entries)) {
    // Reject traversal (`..` segments) and absolute paths (POSIX `/`, Windows `C:\`)
    const segments = name.split(/[\\/]/)
    if (segments.includes('..') || path.isAbsolute(name)) {
      throw new Error(`Unsafe path in archive: ${name}`)
    }
    totalSize += entries[name].size ?? 0
