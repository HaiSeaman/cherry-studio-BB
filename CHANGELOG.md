...
**1. FM 电台四大板块（界面与数据源重构）**

- 一级 tab 改为「国内 / 国外 / 搜索 / 收藏」，国内与国外下各挂「综合 / 音乐」二级 tab，共四个板块；二级 tab 带实际可播条数徽标。
- 数据源由 RadioBrowser 在线拉取改为**打包内置精选清单**：新增 `services/stations.json`（121 台：国内综合 30 / 国内音乐 31 / 国外综合 30 / 国外音乐 30）与 `services/radioBoards.ts`（转 `RadioStation` + 板块 key 映射表 `BOARD_KEYS` / `boardKeyOf()`）。板块列表零网络依赖，断网也能听。
- 电台行升级：台名按「 (」拆主副标题，新增地区/风格徽标（如「国家台」「香港」「巴洛克」）与频道简介一行，编码格式（MP3 / HLS / AAC）一并展示。
- 移除「热门」「中港音乐」两个旧 tab 及其在线拉取链路（`getTopStations` / `getCnHkMusicStations` / `getStationsBySource` / 内置兜底数组 / 7 天列表缓存）；`radioApi.ts` 只保留搜索链路，`radioCache.ts` 只保留 ✕ 隐藏清单。搜索 tab 能力不变（按名称 / 国家 / 标签三模式）。
- 自定义电台并入全部四个板块列表尾部（Redux `customStations`，上限 500、按 url 去重）。

**2. m3u8 直播流改用 hls.js 播放（关键修复）**

- 根因：Chromium 的原生 `<audio>` **不支持 HLS**，而精选清单里有 **31 台**是 m3u8 流（CCTV 系列、香港电台、NHK、KBS、BFM 等）。旧实现直接过滤掉这类电台，等于丢掉全部国家级电视/电台直播源。
- 修复：`audioEngine` 接入 hls.js（复用项目已有依赖，与 IPTV 同一套方案），HLS 流挂载到共享 `<audio>`（走 hls.js 的 MSE 通道），致命错误转发为 error 事件走既有的"失败自动换台"链路，切换归属 / 停止时销毁实例；对外的 `load/play/pause/stop` API 不变。
- 判定收敛到 `radioBoards.isHlsStation(url, codec?)` 单一入口：先看电台数据里的 codec，再查全板块 HLS 地址名单，最后用 `.m3u8` 后缀兜底（覆盖用户自定义与搜索结果）。**RTHK（`stm.rthk.hk/radio1`）、商业电台 881、新城电台等 HLS 流地址里没有 `.m3u8` 字样**，只靠 URL 后缀会漏判，必须查名单。

**3. 双轴代码审查修复（规范 / 需求两轴并行审查，7 项）**

- **自定义电台删不掉**：自定义台与板块台 url 撞车时，去重保留板块版导致列表判定为"非自定义"，✕ 变成隐藏而非删除。改为自定义优先：先剔除撞 url 的板块台再追加自定义（总数不变、该 url 只出现一次、用户命名保留、✕ 可正常删除）。
- **二级 tab 数量徽标不准**：改为该板块**实际可播条数**（含自定义、扣除被 ✕ 隐藏的），与列表严格一致。
- **HLS 判定重复三处**：`radioBoards` / `audioEngine` / `playerStore` 各写一遍正则，易漂移。收敛为唯一入口，播放引擎只认调用方给出的 `meta.hls` 提示，不再自己解析 URL 语义。
- **字符串拼接 + 类型断言**：`${tab}_${sub} as BoardKey` 绕过类型检查，且对 `tab` 的并列分支散落四处。改用 `BOARD_KEYS` 映射表 + `boardKeyOf()` + 类型收窄。
- **命名歧义**：组件里的 `sub`（二级板块）与电台行里的 `sub`（台名副标题）同名异义，分别改名 `boardSub` / `title`+`subtitle`。
- **搜索中切板块被转圈遮挡**：转圈限定仅在搜索 tab 显示（板块是内置数据，根本不需要 loading）。
- 补齐板块数据单测覆盖（映射表一致性、HLS 判定三路径、合并语义），删除无消费方的 `RawStation.id` 字段。

**4. 桌面挂件**

- FM 挂件（`widgets/music/FmRadioView.tsx`）**功能与逻辑零改动**：仍由「自定义电台 + Dexie 收藏」驱动，新板块的电台通过收藏机制自动流入挂件网格。

**质量验证**：tsgo web 类型检查 0 错误；改动文件 biome format / biome lint / oxlint / eslint 全绿；受影响模块单测 11 文件 **97 项 0 失败**（音乐模块 7 文件 69 项 + 便签/挂件相关）；完整构建通过；打包产物已生成。
...
- **单击画面 = 暂停/继续**：左键点一下暂停、再点一下继续（直播与本地视频通用；播放失败态点一下 = 手动重试）。单击延迟 250ms 生效以与双击全屏消歧——双击仍是全屏/退出全屏，不会"顺手暂停"（成熟播放器标准做法）；
- **右上角新增 ✕ 关闭按钮**：点击彻底停引擎并掐断网络流（`removeAttribute('src')`+`load()`，非仅暂停），回到"从列表选择"待机画面；修复此前一旦打开电视/本地视频就无法关闭、只能退出软件的问题。配套防御：关闭后浏览器异步补发的 `pause` 事件与旧引擎迟到错误不再把待机态翻成"已暂停"（防黑屏）。

**2. 打卡：备注功能 + 图标色板扩容**

- **备注（可选）**：添加/编辑习惯弹窗新增备注输入框（≤100 字，"每天至少 2000ml"这类具体要求）；展示于单习惯详情页（习惯主题色引用式卡片）、习惯管理列表（灰字省略）、弹窗实时预览三处；留空不落库、清空保存真正删字段；
- **跨设备同步携带备注**：`SyncHabitDto` 补 `note` 字段全链路（导出/导入/合并）；远端（旧版手机端 schema）未携带时不抹掉本地已有备注；
- **图标 16 → 40 个、颜色 10 → 24 个**（满足"翻一倍以上"）：图标按生活场景排列且只用 Unicode ≤13 字符（Windows 10 表情字体兼容，不出现方框）；色板按色系排布（红橙→金绿→青蓝→紫粉→棕灰），浅色/深色主题下均清晰；图标网格 10 列布局 + 160px 兜底滚动，弹窗小窗不撑爆。

**3. 三线代码审查修复（规范轴 / 需求轴 / 过度设计轴 + 模块深审，全部逐条人工核实）**

- 【数据正确性】**自动重连误杀自愈流**：重连等待期内引擎自愈出画后，未撤销的退避定时器会把健康流销毁重启 → `playing` 事件监听中撤销定时器并复位重试状态；
- 【数据正确性】**自动重连违背暂停意图**：用户暂停后直播流断，自动重连会重新起播 → 暂停态下直接进失败态，等用户手动处理；
- 【数据完整性】打卡格子快速连点存在读-改-写竞态 → `toggleRecord`/`setSkip` 包进 Dexie 事务；电视历史记录裁剪同理加事务；
- 【数据完整性】打卡备份导入不校验日期 → 新增"记录日期必须落在对应习惯 [创建日, 今天] 窗口"校验，未来/创建前日期在写入前拦截；
- 【健壮性】极速连拖两批视频触发 `&path` 唯一索引冲突且 Promise 静默拒绝 → 冲突容忍（已入库部分保留）+ 入口统一报错提示；添加习惯保存失败不再静默（弹窗保留 + 错误提示）；
- 【历史遗留】主进程 electron 桩补 `app.on/once/off`（BrowserServer 单例模块加载期注册监听导致 MCPService 测试全挂）；aiCore StreamEventManager 过时断言对齐现行语义（finish 事件转发保留 totalUsage）；2 处历史 lint 报错清零。

**4. 过度设计轴瘦身（Ponytail，净减约 60 行）**

- 删除 `muted` 死参数链（IptvPage→PlayerArea→PlayerControls 硬编码 false，音量 0 即静音语义）；
- 手写闰年/月份天数表改用平台 `new Date(y, m, 0)`；两处重复的中文星期数组合并为 `calendar.WEEK_DAYS_CN`；统计模块内联重写的周几计算复用既有 `weekdayOf`；
- 收回 6 处仅内部使用的导出（`PlayerStatus`/`MonthDay`/`YearHeatCell`/`YearHeatData`/`MonthRecords`/`HISTORY_LIMIT`）、删除无人传递的 `HeatmapGrid height` prop、频道项杂散 JSX 空格、SyncAdapter 文件尾"防未使用告警"的假导出 `Table`；
- `play`/`stop` 共用拆卸序列抽取 `teardown()`；`onProgress`/`onResizeEnd` 由"可选但人人传"收紧为必填；导出备份 `URL.revokeObjectURL` 延迟回收防个别环境下载被掐。

**质量验证**：Vitest 全仓 **3840 项测试 0 失败**（本版本净增 20+ 项：重连边界 / 备注读写 / 备份日期窗口 / 图标色板数量达标等）；TypeScript 类型检查全绿；oxlint 0 告警、eslint 0 error。

## [1.9.2] - 2026-09-06

### 核心主题：电视页满版布局重塑（贴边 1:8:1 + 三栏可拖拽调宽）+ 本地视频播放器（方案 A 原生引擎）+ 闹钟关铃彻底修复 + 三轴代码审查加固

**1. 电视详情页（IPTV）满版布局重塑**
- 修复页面容器缺 `width:100%` 导致整页被压成窄条、播放器只占小空间的根因：页面现铺满窗口内容区，播放器贴边满版（与知识库页同款布局）；
- 三栏按 **1:8:1**（分组栏 10% / 播放器 80% / 列表栏 10%）对齐，比例按百分比记忆，任何窗口尺寸不走样；
- 左右两根**竖向拖拽手柄**（悬停高亮）可调整三栏宽度，占比持久化（6%~30% 夹取），带卸载防泄漏清理与交互测试。

**2. 本地视频播放器（与 IPTV 同屏融合，原生引擎方案）**
- 右栏新增「电视台 / 本地视频」双 Tab；「添加本地视频」支持多选，`&path` 唯一索引自动去重（Dexie v16 新表 `iptv_locals`，纯追加无迁移）；
- **拖拽视频文件进播放器即自动加入并起播**（Electron 41 `webUtils.getPathForFile` 取路径）；
- 腾讯视频式控制条：**进度条拖动跳转 + 时间显示、上一个/下一个、倍速菜单（0.5x~3x）、播放模式（顺序/单曲循环/随机）、画面顺时针 90° 旋转（点一次转一次）、截图、画中画**；
- **断点续播**：播放中每 3 秒存档，>5s 且未临近片尾自动从上次位置继续并提示；列表项显示"已看 X / Y"；
- 播完自动连播按模式接续（顺序播到末尾停止）；键盘快捷键：空格播放/暂停、←→ 快退快进 5s、↑↓ 音量；
- 本地文件强制原生引擎（防 `.flv` 误路由直播引擎）、失败不自动重连（直接失败态 + 手动重试）；单 video 元素复用，零新增依赖、安装包零增大。

**3. 效率助手闹钟「关不掉铃声」彻底修复（三重根因）**
- 点「关闭闹钟」后会自动"接着响下一个排队的" → 现在点一次**整批清空、立刻安静**；
- 同一闹钟在 90 秒补触发窗口内每秒重复入队（页面数据未回流时） → 触发标记**立即合并进内存**，绝不重复响；
- 音频环境异常导致停止流程卡死 → 全链路容错；停止时立即静音主增益，消除约 1.2s 余音。

**4. 严重崩溃修复（老存档缺新字段）**
- 修复电视页打不开报 `Cannot read properties of undefined (reading 'icon')`：redux-persist 装回老存档整体替换切片导致新增字段缺失 → 接入 `mergeDefaults` 启动补默认值机制（与 shortcuts 同套路，未来新字段自动愈合），控制条防御式查表 + 倍速防非数字双保险。

**5. 三轴代码审查加固（规范轴 / 规格轴 / 过度设计轴，共 30+ 项发现逐项裁决）**
- 收敛 5 处重复设计：`VIDEO_EXTENSIONS` / `isLocalUrl()` / `hasResumePoint()` / `clampRate()` / `clampPanePercent()` 单点化；
- 修复拖拽手柄监听器泄漏、进度条拖动与播放位置"打架"（拖动中只跟手指，松手才 seek）；
- 修正 3 个历史遗留坏测试（对齐实现既有语义），全仓格式化遗留漂移清零。

## [1.9.1] - 2026-09-06

### 核心主题：IPTV 1:8:1 大屏三栏重塑 + 效率助手置顶防遮挡闹钟 + 全仓深度性能瘦身与 50+ 处缺陷彻底修复

**1. 电视详情页（IPTV）1:8:1 大屏三栏重构**
- 布局彻底颠覆：由原先左右颠倒重构为【左：全部频道/收藏(flex:1) - 中：超大播放器(flex:8) - 右：电视台播放列表(flex:1.2)】三栏架构；
- 大屏霸屏体验：大屏及窗口最大化时，中央主播放器占据 80% 视觉中心，全屏模式下 100% 铺满视口；
- 细节优化：播放器中心引导文案同步纠正为"从右侧播放列表选择频道，开始观看"。

**2. 效率中控台闹钟关闭提醒 UI 彻底重构（防遮挡、强感知、秒响应）**
- 全新创建 `AlarmRingingBanner` 全屏置顶浮层（`z-index: 99999`），彻底根治原关闭按钮被列表挤出视口或遮挡的问题；
- 晨间绿洲亚克力玻璃拟态 + 呼吸红光脉冲铃铛动效 + 超大珊瑚红渐变防误触按键；
- 独家全键盘一秒关闹钟：全局监听 `Escape`（ESC 键）与 `Space`（空格键），闹钟响时敲键盘瞬间秒关；
- 卡片内部时钟下方新增常驻警告条，并注入 `stopPropagation` 彻底根治"点击没反应"。

**3. 全仓深度代码审计与致命 BUG 修复（P0 / P1）**
- 修复 `messageThunk.ts` 分支话题克隆整行 put 覆盖导致标题、类型、时间戳抹空的数据丢失隐患；
- 修复 AI 工具流式调用 `textBuffer` 漏清导致同一流二次 finish-step 重复执行工具的问题；
- 修复通用弹窗 `{...rest}` 覆盖调用方 `onOk/onCancel` 确认回调的隐患；
- 消除 MCP 浏览器服务进程级监听器累积泄漏（单例化）；
- API Key 及 Prompt 全文日志脱敏；
- 修复模型选择弹窗共用 TopViewKey 导致 Promise 悬空冲突。

**4. 全仓瘦身、算法优化与死依赖清理（Ponytail Audit）**
- 彻底移除全仓 0 引用的 30MB+ 巨型死依赖 `pdfjs-dist`；
- 消息指纹比对 O(n)→O(1) 优化，长会话流式渲染打字流畅度大幅提升；
- 消息过滤管线消除 O(n²) 循环嵌套，改用 Set 单趟线性扫表；
- 消除三大备份设置页（S3/Local/WebDav）重复的 40+ 行下拉选项，抽取 `backupOptions.ts` 单点收敛；
- 修复 `runAsyncFunction` 注入全局异常捕获日志防御；
- 全仓 oxlint 严格模式 1268 文件 0 告警 0 错误，TypeScript 全绿通过。
...
- **点击复制不再自动置顶**：同内容再次复制（含点击历史条目复制回剪贴板）时，保留原时间戳与列表位置，不再刷新时间、不再提到第一位——除固定/收藏区在前外，其余严格按复制时间顺序排列
- **收藏与置顶双保护**：【上限】（条数/天数修剪）与【清空】两条清理链路统一为「收藏或置顶的消息永不清理」；上限面板、清空确认弹窗文案同步明示双保护
- **双击查看完整内容**：鼠标左键双击任一历史条目，弹出小窗显示完整内容（文本/色号全文、富文本保留换行的纯文本、图片原图、文件路径列表），窗内文字**可选中复制**，另设「复制/关闭」按钮；单击复制采用 300ms 延迟执行以区分双击，避免「只想看详情却被覆盖剪贴板」

**2. 全仓 6 模块并行审查（主进程服务 / 剪贴板挂件 / 渲染核心层 / 页面组件 / 共享包·AI核心 / MCP·依赖）共修复 20 处问题**

**高危（功能失效/数据丢失/崩溃）**
- 修复 Hub `worker.ts` 模板字符串内混入 TS 语法 `(error as …)`：该字符串被 `new Worker(src,{eval:true})` 当纯 JS 执行，**打包产物中 Hub exec/invoke 100% 失败**（vitest 因拦截 worker 属假绿），已改纯 JS 写法并用 Node 实测验证
- 修复桌面挂件 2 秒「假未连接」：收到快照/更新后未清除超时定时器，挂件打开 2 秒必被切成未连接
- 修复剪贴板清空后再复制同内容无法重新入库（`lastCheapKey` 闸门清空后不重置）
- 修复输入框发送后 500ms 二次清空会吞掉用户立即开始的新输入
- 修复翻译/检查 API 完成后 abort 注册永久残留（模块级 Map 无界增长的内存泄漏）
- 修复 DXT 上传临时文件成功后不删除（每次上传泄漏一个文件）

**中危（边界 bug / 资源泄漏 / 安全）**
- `ConfigManager.subscribe` 返回解绑函数；划词助手反复开关不再累积 5 组配置订阅且 stop 清空预载窗口队列
- 修复主窗口重建时 webview `onBeforeSendHeaders` 重复注册累积
- MCP 截图 / 网络抓取 / 滴滴请求补 30s 超时（页面挂起不再永久阻塞）
- didi-mcp 由明文 `http://` + URL query 传 key 改为 **HTTPS + header 传 key**（杜绝 key 泄漏到日志/代理）
- MCP OAuth 超时后 `waitForAuthCode` 不再永久挂起（新增 `abort()`），端口占用时 `close()` 不再产生 unhandled rejection
- 代理配置并发时「静默丢失」改为「最新请求获胜」重放
- 移除 `abortTool` 上从未使用的 `IpcMainInvokeEvent` 参数（消除 `null as any` 类型谎言），并防住 IPC 直接绑定导致的参数错位回归
- 迷你窗口 `removeAllListeners` 改精准清理，不再误伤同通道其他监听
- 消息列表静默搜索标志改 `useRef` 持久；Markdown 流式内容含/不含 `<style>` 的渲染残留修复（移入 useMemo）
- 图片捕获字体样式由「追加」改「覆盖」并在 finally 移除（反复截图不再膨胀）

**死代码清理**
- 删除零引用 `buildGenerateTextParams`（仅 re-export 且只是包装）与 `extractFileContent`（0 调用）及连带失效 import

**依赖核查结论**：全仓无可移除的无用依赖——逐个 grep 验证所有候选（fuse.js/fuzzysort 不在 package.json、jest-styled-components 用于测试、code-inspector-plugin 用于 dev 构建等）均有真实引用。

**3. 质量验证**

- Vitest 全量：主进程 27 文件/328 测试、渲染进程 182 文件/2956 测试、AI 核心 11 文件/360 测试、共享包 4 文件/89 测试，**合计 3733 项全部通过（0 失败）**
- TypeScript Main / Web 对改动文件零错误（既有无关文件错误与基线一致）；oxlint 对全部改动文件 0 error
- 二次审查抓回 3 处自引入问题（`abortTool` IPC 绑定参数错位、OAuth close 日志类型、死代码遗留 import）并全部修复
