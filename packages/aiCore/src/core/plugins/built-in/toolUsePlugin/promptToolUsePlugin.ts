...
function defaultParseToolUse(content: string, tools: ToolSet): { results: ToolUseResult[]; content: string } {
  ...
  // Collect ALL matches first. Using exec() in a loop while also mutating
  // contentToProcess (replace) desynchronizes the regex's lastIndex and
  // silently drops every tool use after the first.
  const matches = Array.from(contentToProcess.matchAll(toolUsePattern))
      ...
      return new TransformStream<TextStreamPart<TOOLS>, TextStreamPart<TOOLS>>({
        async transform(
            ...
            if (tools && Object.keys(tools).length > 0 && !context.hasExecutedToolsInCurrentStep) {
              ...
              if (validToolUses.length > 0) {
                ...
                const executedResults = await toolExecutor.executeTools(
                  ...
                  tools,
                  controller,
                  // Wire the stream's abort signal so user cancellation stops
                  // in-flight tool execution instead of letting it run to completion.
                  (controller as any).signal as AbortSignal | undefined
                )
          ...
          if (chunk.type === 'finish') {
            const accumulated = context.accumulatedUsage
            // Only override the provider's authoritative totalUsage when we
            // actually accumulated step-level usage; otherwise a provider that
            // reports usage only at the end would be zeroed out.
            const hasAccumulated =
              accumulated !== undefined &&
              (accumulated.totalTokens > 0 || accumulated.inputTokens > 0 || accumulated.outputTokens > 0)
            controller.enqueue({
              ...chunk,
              totalUsage: hasAccumulated ? accumulated : chunk.totalUsage
            })
            return
          }
