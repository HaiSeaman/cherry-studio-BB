...
export class ToolExecutor {
    ...
    for (const toolUse of toolUses) {
      ...
      try {
        ...
        const result = await tool.execute(toolUse.arguments, {
          toolCallId: toolUse.id,
          messages: [],
          // Wire the outer stream's abort signal so stopping the stream also
          // interrupts in-flight tool execution.
          abortSignal
        })
  ...
  formatToolResults(executedResults: ExecutedResult[]): string {
    return executedResults
      .map((tr) => {
        if (!tr.isError) {
          return `<tool_use_result>\n  <name>${escapeXml(tr.toolName)}</name>\n  <result>${escapeXml(tr.result)}</result>\n</tool_use_result>`
        } else {
