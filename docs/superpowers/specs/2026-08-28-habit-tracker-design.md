...
日期：2026-08-28（v2 修订）｜2026-08-29（v3 增补：自然年统计 + 排版重设计）
状态：已实现并发布（v1.7.2）
...
>
> v3（1.7.2）增补：① 统计视图新增【自然年统计】模块（年份切换 + 习惯 chips + 整年热力图），
> 口径见 `yearWindow` / `yearlyCheckinStats` / `yearlyHeatCells`；原「年度热力图」（多习惯完成率视图）
> 已移除，其职责由自然年统计承接。② 打卡页排版/UI 重设计：eyebrow 标题体系、卡片 `minmax(0,1fr)`
> 防挤压、热力图格子最小 14px + 横向滚动、趋势图渐变面积与参考线、对比条表头。
...
interface Habit {
  ...
  note?: string           // 备注（可选，1.9.3 追加）：具体要求如"每天至少 2000ml"；老数据无此字段
...
// note（1.9.3 追加）：
// - 非索引可选字段，无 schema 升级；留空不落库，清空保存写 undefined 真删字段
// - 展示位：单习惯详情页（主题色引用式卡片）/ 习惯管理列表（灰字省略）/ 表单实时预览
// - 跨设备同步：SyncHabitDto.note 全链路携带；远端未携带（旧 schema）时保留本地值不抹除
// - 备份导入：isValidHabitRow 校验 note 为可选字符串；记录日期须落在 [创建日, 今天] 窗口内
```
...
│       右侧：当前连续 / 最长连续
└── 统计视图（4 模块）：
    ├── 每日完成率趋势折线图（30/90/全年切换；渐变面积 + 参考虚线 + 当日完成率标注）
    ├── 各习惯完成率横向条形图（按强度指数排序，含表头）
    ├── 自然年统计：年份切换（创建年→今年，默认今年）+ 习惯 chips 切换
    │   + 选中习惯当年打卡天数；整年格子 3 态：打卡=习惯色深格 / 跳过=浅色 / 未打卡=空白，
    │   今天描边，悬停显示「M月D日 · 状态」；口径见 stats.ts yearWindow/yearlyCheckinStats/yearlyHeatCells
    └── 单习惯详情（点击习惯行进入）：
...
- 点击习惯名称：进入单习惯详情
- 添加/编辑习惯：名称 + 备注（可选，1.9.3 追加）+ emoji 图标（40 个，仅用 Unicode ≤13 字符保证 Win10 表情字体渲染）+ 色板选色（24 色，按色系排布）；单习惯详情页头部下方展示备注引用卡（1.9.3 追加）；删除走归档（保留历史数据），习惯管理子视图提供「已归档」列表：恢复 / 彻底删除（彻底删除需二次确认，连带删除该习惯全部记录）
...
- **当前连续**：从今天往前数连续 done 天数；**今天还没打不算断**——若今天无记录，则从昨天往前数（今天还没过完，不能判死刑）。skip 日计入连续天数（「连续」的语义是**未中断天数**，跳过不断卡自然不算中断），继续往前数
- **最长连续**：历史最长连续段（口径同上，skip 计入未中断天数）
