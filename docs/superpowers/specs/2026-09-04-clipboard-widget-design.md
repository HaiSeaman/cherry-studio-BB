...
日期：2026-09-04
状态：✅ 已在 v1.8.3 落地；v1.8.4 三项交互迭代（去重不提升/双保护/双击详情，见 §7）
...
- 点击条目：`Clipboard_CopyItem` → 顶栏下方短暂浮现"已复制"提示（1.2s 自动消失）；单击复制延迟 300ms 执行，若 300ms 内触发双击则取消复制
- **双击条目（v1.8.4 新增）**：弹出详情小窗显示完整内容（文本/色号全文、富文本保留换行的纯文本、图片原图、文件路径列表），窗内文字可选中复制，附「复制/关闭」按钮，点遮罩或关闭按钮退出
...
1. **去重**：full 指纹命中 → **保留该条原时间戳与列表位置，不提升、不刷新 ts**（v1.8.4 起变更：点击复制不再自动置顶；全新内容才插到最前按时间排序）
2. **排序**：固定/收藏区在前（组内按 ts 倒序），历史区按 ts 倒序；点击复制不改变顺位
3. **上限与清空双保护（v1.8.4 变更）**：【清空】与【上限】修剪（条数/天数）统一保护「收藏 **或** 置顶」的条目，永不清理、不受限制；清空后重置轮询闸门，再次复制同内容可正常重新入库
