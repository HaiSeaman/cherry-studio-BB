...
> ⚠️ **已下线（v1.11.0）**：本文档描述的「电视」（IPTV）功能已整体移除 —— 页面与路由删除、专用依赖（`mpegts.js` / `iptv-playlist-parser`）卸载、Dexie 五张 `iptv_*` 表由 `version(17)` 删除。
> 本文档**仅保留作历史设计记录**，其中提到的代码路径、依赖与数据库表在现行代码中**均已不存在**。功能现状见 [docs/wiki/](./wiki/) 与 [CHANGELOG](../CHANGELOG.md)。
...
> **1.9.3 追记（播放器交互增强）**：
> - **单击视频区 = 暂停/继续**（失败态点一下 = 手动重试）。单击延迟 250ms 生效，与双击全屏消歧（双击取消未触发的单击），成熟播放器标准做法；
> - **视频区右上角新增 ✕ 关闭按钮**：调 `playerStore.stop()`——停引擎 + `removeAttribute('src')`+`load()` 真断流（非仅暂停），回待机画面；配套防御：关闭后浏览器异步补发的 `pause` 事件与旧引擎迟到错误不把 idle 翻成 paused/failed；
> - 播放器内核状态机加固：重连等待期内流自愈（`playing`）时撤销待执行的重连定时器（防误杀健康流）；暂停态断流直接进失败态（自动重连不得违背暂停意图重新起播）；
> - `muted` 参数链已删除（`muted` 恒 false、音量 0 即静音语义），`play`/`stop` 拆卸序列收敛为 `teardown()`。
